...
Có hai cách kiếm tiền từ giá tài sản, và chúng khác nhau tận gốc về mặt toán học. Q-world (cuốn chị em) trả lời câu hỏi "derivative này giá bao nhiêu **hôm nay**" bằng no-arbitrage: nếu payoff có thể replicate bằng một danh mục tài sản giao dịch được, giá derivative phải bằng chi phí replication — nếu không sẽ có arbitrage. Người Q-world không cần dự báo drift thật của underlying; họ cần spot/forward, discount curve, dividend hoặc borrow, vol surface và một model đủ phù hợp để price và hedge. Trong Black–Scholes lý tưởng, drift thật $\mu$ biến mất khỏi giá option, nhưng model risk và rủi ro hedge không vì thế mà biến mất.

P-world chơi trò khác: **ước lượng phân phối có điều kiện ngoài đời thật, hoặc ít nhất một hàm hữu ích của nó, rồi biến dự báo thành P&L dương sau chi phí**. Không phải chiến lược nào cũng dự báo toàn bộ phân phối hay trực tiếp ước lượng $\mu$; nhiều chiến lược chỉ xếp hạng cross-section, dự báo sign, volatility, regime hoặc relative value. Và không cần "đúng thường xuyên hơn sai": một chiến lược thắng ít nhưng payoff khi thắng lớn vẫn có kỳ vọng dương. Điều cần là conditional edge và portfolio construction tốt, không phải hit rate trên 50%.

"P" là **physical measure** (real-world measure, và trong nhiều tài liệu còn gọi là historical measure): xác suất của các kịch bản trong thế giới thật, được ước lượng từ dữ liệu và giả định mô hình. **Historical estimation** là một cách ước lượng P từ dữ liệu quá khứ, không phải một measure thứ ba. "Q" là một **equivalent martingale measure**: dưới money-market numéraire, discounted gains process của tài sản giao dịch được là martingale. Vì vậy total expected return của một tài sản không dividend trong mô hình đơn giản bằng $r$ dưới Q; với dividend, carry hoặc numéraire khác, phát biểu drift phải điều chỉnh.

Trong GBM một yếu tố,

$$\frac{dS_t}{S_t}=\mu\,dt+\sigma\,dW_t^{\mathbb P},$$

market price of risk là
$$\lambda = \frac{\mu - r}{\sigma} = \frac{0.08 - 0.04}{0.20} = 0.20.$$

Đặt $dW_t^{\mathbb Q}=dW_t^{\mathbb P}+\lambda\,dt$ thì Girsanov cho

$$\frac{dS_t}{S_t}=r\,dt+\sigma\,dW_t^{\mathbb Q}.$$

**Radon–Nikodym derivative không phải chính $\lambda$**; nó là density process chứa $\lambda$, chẳng hạn với $\lambda$ hằng:

$$\left.\frac{d\mathbb Q}{d\mathbb P}\right|_{\mathcal F_T}=\exp\!\left(-\lambda W_T^{\mathbb P}-\tfrac12\lambda^2T\right).$$

Con số $\lambda=0.20$ nói excess drift bằng $\lambda\sigma=4\%$. Trong mô hình nhiều yếu tố, $\lambda$ là vector và mỗi tài sản tải lên các risk factor khác nhau; trong thị trường incomplete, Q còn có thể không duy nhất. Q không "đặt $\lambda$ về 0" — phép đổi measure thay đổi trọng số xác suất sao cho risk premium không còn nằm trong drift định giá. P-world thì cố ước lượng chính các risk premium và mispricing mà bước đổi measure đã hấp thụ.

Người P-world dùng thống kê để ước lượng conditional edge từ dữ liệu, chấp nhận sai số lớn, rồi dùng breadth — nhiều quyết định đủ khác nhau — để biến edge nhỏ thành P&L có signal-to-noise tốt hơn. Breadth chỉ giúp khi các cược thật sự mang thông tin bổ sung; nhân bản cùng một exposure không tạo thêm edge.

Ba sự thật nền tảng định hình toàn bộ nghề — ghi nhớ trước khi học bất kỳ kỹ thuật nào.

**Thứ nhất: tín hiệu cực yếu.** Thị trường tài chính là hệ thống gần-hiệu-quả: nhiều người cùng săn một pattern, nên edge dễ decay khi vốn đổ vào. **Information coefficient** ($IC$) là correlation giữa forecast và realized return; trên equity cross-section người ta thường dùng Spearman rank-IC. Mức 0.02–0.05 có thể đáng kể trong một số horizon/universe, nhưng không phải benchmark phổ quát — ý nghĩa của nó phụ thuộc sampling, turnover, decay và chi phí. Nếu $IC=0.03$ là **Pearson correlation** trong một hồi quy tuyến tính đơn đã chuẩn hóa, $IC^2=0.0009$ có thể đọc như $R^2$ in-sample. Với rank-IC, bình phương Spearman correlation không trực tiếp là tỷ lệ variance return được giải thích. Dù vậy, cả hai cách đều nhấn mạnh cùng điều: signal mỗi quan sát rất mỏng.

**Thứ hai: nhiễu thống trị ở mọi thang thời gian bạn quan tâm.** Nếu annual excess return được giả định Gaussian, stationary và có mean bằng đúng một annual volatility, thì $SR=1$ cho xác suất một năm excess return âm là $\Phi(-1)\approx15.9\%$. Đây là xác suất mỗi năm dưới mô hình, không có nghĩa cứ đúng sáu năm sẽ có một năm lỗ; serial dependence, skew và fat tails còn làm thực tế khác Gaussian. Phân biệt may mắn với kỹ năng vì thế đòi hỏi lượng dữ liệu lớn, trong khi dữ liệu tài chính ngắn và non-stationary.

**Thứ ba: kẻ thù số một là chính mình — overfitting.** Với đủ tham số và đủ lần thử, bạn gần như luôn tìm được một backtest đẹp dù không có edge thật; nhiều chiến lược như vậy sẽ suy yếu hoặc chết khi chạy live. Phần lớn kỹ thuật nâng cao của P-world (purged cross-validation ở Ch9, deflated Sharpe, combinatorial backtest) là kỷ luật chống tự lừa dối.
...
Đọc bảng này như một **upper bound lý tưởng**. Với 100 cược iid một năm, edge 2% cho reward-to-risk 0.2; 2.500 cược iid cho 1.0; 10.000 cho 2.0. Nhưng "10 cổ phiếu mỗi ngày" không mặc nhiên là 10 cược độc lập, và high-frequency fills cũng không phải các đồng xu cùng payoff. Correlation, capacity, inventory risk, fees và market impact khiến breadth hiệu dụng thấp hơn số giao dịch rất nhiều. Số lượng chỉ biến edge thành chiến lược mạnh khi mỗi quan sát bổ sung thông tin đủ độc lập và edge sống sót sau chi phí.
...
Chỉ 100 ván thì nhà cái vẫn lỗ khoảng 39% số chuỗi — edge bị nhiễu nuốt. Ở 10.000 ván iid, xấp xỉ Gaussian đưa xác suất lỗ xuống khoảng 0.35%; ở 100.000 ván nó cực nhỏ nhưng vẫn không bằng 0. Đây chính là quy luật $\sqrt N$ dưới edge ổn định và independence. Quant fund chỉ giống casino ở phép cộng kỳ vọng; khác biệt sống còn là xác suất và payoff thị trường thay đổi theo thời gian.
...
**Chỗ gãy 2 — edge không hằng số.** Đồng xu 51/49 trong ví dụ không đổi, còn edge thị trường có thể decay khi bị khai thác. McLean & Pontiff (2016) nghiên cứu 97 predictors và báo cáo return thấp hơn 26% out-of-sample, thấp hơn 58% sau publication; phần suy giảm họ quy cho publication-informed trading vào khoảng 32 điểm phần trăm. Đây là bằng chứng theo mẫu nghiên cứu, không phải định luật rằng mọi anomaly đều giảm đúng một tỷ lệ cố định.

Phiên bản lý tưởng của **Fundamental Law of Active Management** là

$$IR \approx IC \times \sqrt{BR}.$$

Phiên bản có constraints thêm **transfer coefficient** $TC$:

$$IR \approx TC\times IC\times\sqrt{BR}, \qquad -1\le TC\le1.$$

$BR$ là số quyết định thông tin độc lập, không phải số cổ phiếu nhân số kỳ một cách cơ học; $TC$ là correlation đo mức position thực tế bám theo forecast sau constraints và costs. Với portfolio construction hợp lý ta kỳ vọng $0\le TC\le1$; $TC<0$ nghĩa là implementation đang đi ngược forecast. Ví dụ minh họa: nếu $IC=0.025$, $BR=400$ và $TC=0.7$, law cho $IR\approx0.35$. Đây là phép tính planning dưới giả định IC ổn định và breadth độc lập, không phải dự báo Sharpe thực nghiệm của momentum hay một bảo đảm performance.
...
**Return prediction thường có signal-to-noise rất thấp.** Lấy một ví dụ stylized: cổ phiếu vol 2%/ngày có weekly vol xấp xỉ $2\%\sqrt5=4.47\%$ nếu daily returns iid; một strategy có expected weekly alpha 0.3% thì mean-to-vol ratio mỗi tuần là $0.3/4.47\approx0.067$. Điều này không có nghĩa "93% quan sát là noise". Mean chỉ bằng 6.7% của một weekly standard deviation; trong mô hình signal-cộng-noise đơn giản, variance do một signal hằng giải thích cũng không được suy ra bằng phép trừ $1-0.067$. Nếu weekly observations iid, stationary và không có multiple testing, t-stat của sample mean xấp xỉ $t=0.067\sqrt n$. Để đạt $t=2$, cần
$$n = \Big(\frac{2}{0.067}\Big)^2 \approx 900 \text{ tuần} \approx 17 \text{ năm dữ liệu.}$$
Khoảng 900 tuần, tức hơn 17 năm, chỉ để đạt t-stat 2 trong mô hình lý tưởng đó. Autocorrelation, fat tails, non-stationarity và việc thử nhiều signals thường làm effective sample size thấp hơn hoặc ngưỡng bằng chứng cần cao hơn. Cross-sectional breadth có thể bổ sung thông tin, nhưng chỉ trong phần các tài sản không cùng factor/cùng shock; nó không tự động thay thế 17 năm lịch sử bằng 1.000 mã tương quan.

**Sai số Q và P lộ ra theo cách khác nhau.** Q-model có market quotes và no-arbitrage relations làm neo, nên calibration error thường quan sát được ngay. Nhưng calibration không cứu một model sai dynamics: barrier, smile evolution, jumps hoặc correlation sai vẫn tạo hedge P&L thật. P-model thiếu neo contemporaneous tương tự; forecast sai chỉ lộ dần qua noisy realized returns. Cả hai thế giới đều mất tiền khi sai — khác nhau ở loại feedback và tốc độ nhận biết sai số.
...
**Chỉ có một lịch sử.** Ta có các bộ dữ liệu equity Mỹ được tái dựng về thập niên 1920, nhưng chỉ số S&P 500 chính thức ra mắt năm 1957; dù chọn series nào, đó vẫn là một realization từ quá trình không dừng. Bạn không thể chạy lại năm 2008 với điều kiện khác. Điều này khiến mọi khẳng định thống kê trong P-world mong manh hơn trực giác từ thí nghiệm lặp lại.
...
Trước khi học kỹ thuật, phải biết bức tranh: ai cấp vốn, cấu trúc tổ chức nào, và vì sao điều đó định hình loại alpha mà mỗi nơi đi tìm. **Buy-side** quản lý hoặc triển khai capital để tạo return cho nhà đầu tư/chủ sở hữu; **sell-side** cung cấp thanh khoản, execution, financing, research và sản phẩm. Tên gọi không có nghĩa buy-side luôn "mua rủi ro" còn sell-side luôn bán sạch rủi ro. Dưới đây là sáu nhóm stylized, có thể chồng lấn.

**Multi-strategy / multi-manager ("pod shops").** Đại diện năm 2026 gồm **Citadel, Millennium, Point72, Balyasny, ExodusPoint, Schonfeld**. Nhiều PM-led teams chạy book riêng dưới risk management trung tâm; một số platform dùng pass-through fee để chuyển phần lớn operating expenses cho investors. Drawdown limits thường chặt và có thể ở vùng single-digit, nhưng ngưỡng cắt vốn/đóng pod là policy nội bộ, khác theo platform, strategy và mandate — không có quy tắc công khai phổ quát kiểu "−5% cắt nửa, −10% sa thải". Mô hình này tăng ảnh hưởng mạnh trong thập niên 2015–2025, nhưng performance và capital flows vẫn thay đổi theo chu kỳ.

**Quant thuần hệ thống.** Đại diện: **Renaissance, Two Sigma, D. E. Shaw, AQR, PDT, WorldQuant, Man AHL/Numeric, Squarepoint, Qube, XTX**. Medallion của Renaissance có performance lịch sử được báo cáo rộng rãi nhưng không có đủ dữ liệu công khai để dùng như benchmark kỹ thuật. Các hãng trong danh sách cũng không cùng business model: XTX là electronic market maker, AQR/Man quản lý vốn khách hàng, còn các firm khác pha trộn prop và asset management. Điểm chung hữu ích là research và portfolio thường được tích hợp vào platform chung hơn mô hình PM-pod thuần túy.

**Prop trading / market makers.** Đại diện: **Jane Street, Citadel Securities, Optiver, IMC, SIG, Hudson River Trading, Jump, Tower, DRW, Virtu**. Họ dùng firm capital và hạ tầng điện tử để market-make, arbitrage và quản lý inventory trên nhiều venue. Nhiều hãng tuyển mạnh từ toán/CS và đào tạo finance nội bộ. Đây là nơi trực giác breadth đặc biệt quan trọng, nhưng số quote/fill không được đồng nhất với số cược độc lập trong Fundamental Law.

**CTA / managed futures.** Đại diện: **AQR, Man AHL, Winton, Aspect, Transtrend**. Trend following trên futures là strategy cốt lõi, bên cạnh carry và systematic macro. Historical Sharpe thường vừa phải và trend đã tạo crisis alpha trong một số cú sốc lớn, nhưng không phải mọi equity drawdown đều có trend đủ dài để CTA sinh lời; diversification benefit mang tính thống kê, không phải bảo hiểm chắc chắn.

**Asset managers định lượng.** Đại diện: **BlackRock Systematic, AQR, Dimensional (DFA), Robeco Quantitative Investments**. Họ triển khai factor/systematic strategies như value, quality, momentum và low volatility ở horizon dài hơn, thường ưu tiên capacity, tracking error và implementation cost. Tổng AUM của parent firm không nên nhầm với vốn nằm trong một strategy định lượng cụ thể.

**Crypto quant.** Đại diện: market maker chuyên như **Wintermute, GSR, Amber** và desk digital-assets của các hãng lớn. Cơ hội tương đối đến từ trading 24/7, venue fragmentation, funding/basis, khác biệt collateral và market structure — không phải chỉ vì lịch sử ngắn. Chính fragmentation đó cũng tạo exchange, custody, stablecoin, liquidation và counterparty risk lớn hơn thị trường truyền thống.

### Kinh tế học một pod — để hiểu áp lực từ đâu ra

Đây là một **ví dụ stylized**, không phải term sheet đại diện cho mọi platform. Mục tiêu là thấy cơ chế incentive bằng số; allocation, leverage, payout và stop-loss thật đều là thông tin nội bộ và thay đổi theo strategy.

Giả sử PM được quy đổi một **allocation 1 tỷ USD** — cách platform quy đổi risk budget thành "capital" không thống nhất. Pod chạy market-neutral với **gross exposure 3x allocation**: 1.5 tỷ USD long cộng 1.5 tỷ USD short, tổng gross 3 tỷ và net gần 0. Nếu là 3 tỷ long cộng 3 tỷ short thì gross phải gọi là 6x, không phải 3x.

Giả sử mục tiêu return trên allocation là 6%/năm, tức 60 triệu USD P&L, và pool thưởng theo ví dụ bằng 15% net P&L, tức tối đa 9 triệu trước cách chia nội bộ. Hai tỷ lệ này chỉ là input minh họa; payout thực phụ thuộc contract, hurdle, charge và clawback.

Trong mô hình pass-through hoặc chargeback, data, compensation, infrastructure, financing và transaction costs có thể được phân bổ vào economics của pod theo cách khác nhau. Giữ giả định chi phí 8 triệu USD: 60M gross còn 52M trước payout; 20M gross còn 12M. Con số 8M không phải benchmark ngành, chỉ minh họa operating leverage của chi phí cố định.

Giả sử riêng ví dụ này platform bắt đầu de-risk ở drawdown −5% và đóng pod ở −10%. Đây không phải ngưỡng phổ quát. Nếu strategy có expected return 6% và annual vol 4%, Gaussian terminal-return model cho Sharpe 1.5 và xác suất một năm return âm $\Phi(-1.5)\approx6.7\%$. Nhưng $5\%/4\%=1.25$ chỉ chuẩn hóa một **terminal return shock**, không phải xác suất maximum drawdown: peak-to-trough drawdown phụ thuộc cả đường đi, autocorrelation, stop rules và intrayear sampling. Chính khác biệt này giải thích vì sao risk platform theo dõi path và stress exposure, không chỉ annual Sharpe.

Hệ quả văn hóa: cấu trúc thưởng phạt này thường ưu tiên chiến lược **Sharpe cao, drawdown nông, capacity vừa** — stat-arb, relative value, các tín hiệu hoàn vốn nhanh — hơn các strategy cần chịu drawdown dài như trend hoặc value. Một pod với stop-loss ngắn có thể bị đóng trước khi edge horizon dài kịp phục hồi. Đây là mismatch giữa investment horizon và organizational horizon, không phải bằng chứng trend/value là khoa học kém. Nó cũng góp phần tạo crowding trong không gian alpha ngắn hạn mà Ch14 sẽ phân tích.

Một platform collaborative hoặc asset manager dài hạn có horizon đánh giá và cơ chế phân bổ khác, nên có thể chịu một số drawdown lâu hơn pod ngắn hạn. Nhưng họ vẫn có risk limits, redemption pressure và quy trình dừng model; khác biệt là mức độ và horizon, không phải một bên có constraints còn bên kia thì không.
...
**Risk.** Xây risk model, giám sát exposure theo factor, chạy stress test và scenario analysis. Buy-side thường không chịu bank regulatory capital theo cùng cách sell-side, nhưng giám sát chặt drawdown, leverage, liquidity và concentration. Ở pod shop, risk trung tâm thực thi limit và de-risking policy riêng của platform; không có cặp ngưỡng −5%/−10% áp dụng chung toàn ngành.
...
**Các loại dữ liệu theo tần suất**: daily bars (OHLCV — open/high/low/close/volume) → intraday bars (1min/5min) → tick data (từng giao dịch) → order-book data. **L2** thường là depth đã aggregate theo mức giá; **L3/order-level** mới theo dõi từng order/add/cancel/modify tùy venue và feed. Tần suất cao hơn cho phép nghiên cứu edge ngắn hơn, nhưng không bảo đảm alpha tốt hơn và làm chi phí dữ liệu/hạ tầng tăng mạnh.

Hãy hình dung cái tháp này bằng order of magnitude. Một cổ phiếu Mỹ trong một ngày cho một daily bar; bar 1 phút trong phiên 9:30–16:00 cho $6.5\times60=390$ dòng. Tick và order-book message counts phụ thuộc mạnh vào mã, ngày, venue, feed consolidation và việc có giữ mọi update hay không, nhưng thường lớn hơn bars nhiều bậc. Không có hệ số 100–1000 cố định cho mỗi tầng. Điểm bền vững là kiến trúc thay đổi: daily research có thể chạy trên máy đơn, còn low-latency market making cần feed handlers, clock synchronization, replay, colocation và kiểm soát packet loss.
...
1. **Corporate actions**: cổ phiếu split 2:1 → raw price xấp xỉ giảm một nửa nhưng số cổ phần tăng gấp đôi. Dùng **total-return-adjusted prices** hoặc raw price cộng cash distributions để tính investor return; dùng raw executable prices, effective share quantities và corporate-action events để mô phỏng giao dịch. Giữ cả raw lẫn adjusted series và factor tạo ra chúng.
2. **Survivorship bias**: dataset chỉ chứa công ty còn sống hôm nay khiến backtest né bankrupt/delisted names. Mức bias không có một con số cố định; nó phụ thuộc universe, weighting, horizon và delisting treatment. Bắt buộc giữ historical constituents, dead securities và delisting proceeds/returns khi có.
...
5. Ticker tái sử dụng, đổi tên, đổi sàn → cần **security master** với internal instrument/entity ID và các mapping effective-dated sang ticker, CUSIP, ISIN, FIGI. Không identifier vendor nào bất biến tuyệt đối qua mọi corporate action, nên không được dùng một CUSIP/ISIN đơn lẻ làm khóa vĩnh viễn.

**Hai ví dụ điều chỉnh cụ thể** để thấy bẫy vận hành thế nào:

*Split*: giả sử cổ phiếu đóng 500 trước split 4:1 và, bỏ qua overnight market move, mở ex-date ở 125. Chuỗi raw tạo return giả $125/500-1=-75\%$. Back-adjust quá khứ theo factor $1/4$ cho ..., 124.5, 125, 125, 126; return qua split là 0% trong giả định không có overnight move. Ngoài đời giá ex-date có thể khác 125 vì market move, nên adjusted return phải giữ phần biến động kinh tế đó thay vì cưỡng ép về 0. Khi mô phỏng holdings, mỗi cổ cũ cũng phải biến thành bốn cổ mới.
...
*Survivorship bias bằng số*: xét một toy portfolio equal-weight và giả sử 3% universe mỗi năm là bad delistings với loss trung bình 60%, trong khi việc lọc survivors loại đúng toàn bộ nhóm đó. Upper-bound arithmetic contribution bị bỏ qua là $0.03\times0.60=1.8\%$/năm. Đây **không phải ước lượng phổ quát**: acquisition delistings có thể dương, weights thay đổi, loss diễn ra trước delisting và strategy có thể đã bán từ trước. Nếu giữ nguyên vol 18%, thêm 1.8 điểm phần trăm vào mean 5.4% chỉ đẩy Sharpe từ 0.30 lên 0.40; không thể tự kết luận nó thành 0.6–0.7 nếu chưa đo cả tác động lên covariance và crisis exposure. Cách chữa là giữ dead securities và cash/delisting return thực tế, đồng thời audit rule thay thế khi return cuối thiếu.
...
Ba lớp thời gian cần tách rõ. **Economic/event time** là kỳ mà fact mô tả (`period_end=2026-03-31`). **Source-available time** là lúc version trở nên công khai hoặc được vendor cung cấp cho người dùng (`source_available_at=2026-05-15`). **System-ingestion time** là lúc một pipeline cụ thể thực sự nhận version đó (`system_ingested_at`), có thể muộn hơn và cũng có thể là ngày backfill nhiều năm sau. Restatement ngày 20/8 là một version mới với `source_available_at=2026-08-20`; bản công bố ban đầu không được overwrite. Hypothetical PIT backtest tại $t$ lọc theo `source_available_at\le t`; chỉ khi replay đúng trạng thái production lịch sử mới lọc thêm `system_ingested_at\le t`. Schema bitemporal có thể dùng tên khác, nhưng hai ý nghĩa này phải được giữ tách biệt và nhất quán.
...
*Universe PIT bằng số*: S&P 500 thay nhiều constituents theo thời gian, nhưng số thay mỗi năm không cố định. Nếu backtest "long S&P 500 constituents" năm 2010 bằng danh sách hôm nay, bạn chọn trước các công ty sống sót/thành công. Mỗi ngày phải dùng membership effective-dated, đồng thời mô phỏng thời điểm announcement, effective date và khả năng trade; membership được biết trước effective date có thể là signal hợp lệ, còn membership nhìn từ tương lai là leakage.

**Alternative data** gồm transaction panels, geospatial imagery, web/app exhaust, shipping và text từ filings/calls/news. Dữ liệu thường đắt, bẩn, có selection bias, lịch sử ngắn và quyền sử dụng phức tạp. Một feed phổ biến không tự tạo alpha; differentiation nằm ở sourcing, legal/compliance, entity mapping, PIT processing và cách kết hợp với dữ liệu khác.
...
Về mặt "ai trả tiền cho alpha", xét một toy payoff đối xứng: signal panel đúng hướng 60% số events; trade lời 4% khi đúng và lỗ 4% khi sai, trước costs. Khi đó expected return mỗi event là $0.6(4\%)+0.4(-4\%)=0.8\%$. Nếu loss khi sai, market beta, gap risk hoặc sizing khác 4%, công thức $(2p-1)\times4\%$ không còn đúng. Và 60% phải được ước lượng out-of-sample trên PIT data; không thể suy từ một ví dụ nowcast. Alt-data tạo lợi thế khi source, cleaning hoặc latency mang thông tin incremental sau consensus và transaction costs, không chỉ vì con số đến sớm.

## 2.2 Returns — đơn vị tiền tệ của P-world

P-world thường mô hình hóa **returns** thay vì price levels vì returns không phụ thuộc đơn vị giá và thường gần stationary hơn. "Gần" rất quan trọng: mean, volatility, tails và dependence của returns vẫn thay đổi theo regime; một số bài như cointegration, execution và derivatives còn cần price levels trực tiếp.

Price levels thường chứa stochastic trend, nên sample mean/variance của level khó diễn giải như moments ổn định. Điều đó không làm mọi thống kê giá vô nghĩa: spreads, ratios và cointegrating combinations có thể stationary và mang tín hiệu. Return chuẩn hóa thay đổi theo mức giá: +2% có cùng đơn vị tương đối khi price là 5 hay 500, giúp so sánh và gộp tài sản.

- **Simple total return**: $r_t=(P_t+D_t)/P_{t-1}-1$ với cash distribution $D_t$ và không xét withholding tax; nếu dùng total-return-adjusted price $P_t^{\mathrm{adj}}$ thì $r_t=P_t^{\mathrm{adj}}/P_{t-1}^{\mathrm{adj}}-1$. Khi không có distribution, công thức rút về price return $P_t/P_{t-1}-1$.
- **Log return**: $\tilde r_t=\ln(1+r_t)$. Nó cộng được theo thời gian miễn $1+r_t>0$: $\tilde r_{0\to T}=\sum_t\tilde r_t$. Với return nhỏ, $\tilde r\approx r-r^2/2$; với return lớn hai loại khác đáng kể (−50% simple tương ứng −69.3% log).
- **Excess return**: $r_t-r_{f,t}$ với risk-free return cùng kỳ — dùng cho Sharpe và nhiều asset-pricing regressions. **Active return** $r_p-r_b$ so với benchmark mới là đầu vào của information ratio. Alpha là intercept/model residual expected return, nên benchmark của nó phụ thuộc factor model chứ không chỉ risk-free rate.
- **Annualization**: arithmetic mean daily thường nhân 252; volatility và Sharpe chỉ scale theo $\sqrt{252}$ dưới assumptions phù hợp về stationarity/dependence. Geometric CAGR phải compound wealth, không bằng sample mean × 252 nói chung.

Theo thời gian, $P_2=P_0(1+r_1)(1+r_2)$ nên simple returns compound bằng phép nhân, còn $\ln(P_2/P_0)=\tilde r_1+\tilde r_2$ nên log returns cộng chính xác. AR/GARCH có thể dùng simple hoặc log return tùy specification; lợi thế riêng của log là time aggregation. Giữa các tài sản, danh mục với **beginning-of-period weights** $w_{i,t-1}$ và không có external cash flow có simple return $r_{p,t}=\sum_i w_{i,t-1}r_{i,t}$. Log return không cộng tuyến tính ngang tài sản. Nhớ hai câu: **log cộng theo thời gian; simple cộng theo tài sản với weights đúng thời điểm**.

Khai triển $\ln(1+r)=r-r^2/2+r^3/3-\dots$ cho $r=1\%$ được log return $0.995\%$, lệch simple return 0.005 điểm phần trăm. Với $r=-50\%$, log return là $-69.3\%$. Không có ngưỡng 5% phổ quát để hai loại thay thế được: tolerance phụ thuộc bài toán, horizon và độ chính xác; trong P&L/risk aggregation phải giữ convention nhất quán.
...
- Daily mean excess return 0.05% và daily vol 1%. Dưới iid/stationary arithmetic-return convention: annualized mean $=12.6\%$, vol $=15.9\%$, Sharpe $\approx0.79$. Đây không phải CAGR; geometric growth thấp hơn do volatility drag và compounding.
...
Quy tắc $\sqrt t$ đến từ covariance:

$$\operatorname{Var}\!\left(\sum_{t=1}^T r_t\right)=\sum_{t=1}^T\operatorname{Var}(r_t)+2\sum_{i<j}\operatorname{Cov}(r_i,r_j).$$

Nếu returns uncorrelated và có cùng variance $\sigma_d^2$, covariance terms bằng 0 nên volatility là $\sigma_d\sqrt T$; arithmetic mean scale theo $T$, do đó Sharpe scale theo $\sqrt T$. Positive autocorrelation làm horizon variance cao hơn iid scaling, negative autocorrelation làm thấp hơn. Volatility clustering không tự nó tạo return autocorrelation, nhưng làm conditional variance thay đổi và khiến standard errors/forecasts cần model phù hợp. Vì vậy 252 và $\sqrt{252}$ là convention dựa trên assumptions, không phải hằng đẳng thức.

Ví dụ, annualized arithmetic mean return 12%, matching risk-free return 5% và annualized vol 15.9% cho Sharpe $(12\%-5\%)/15.9\%=0.44$. Dùng raw return cho Sharpe sẽ ra 0.75 và trộn compensation của cash với risky excess return. Nhưng information ratio dùng active return so với benchmark, còn alpha dùng factor-model intercept; không phải mọi performance metric đều trừ T-bill theo cùng cách.

## 2.3 Stylized facts — returns thật trông như thế nào

Trước khi model, phải biết đối tượng. Các regularities dưới đây xuất hiện rộng rãi trong nhiều liquid markets, nhưng cường độ phụ thuộc asset, sampling frequency và sample period (Cont 2001):

1. **Đuôi dày (fat tails)**: sample kurtosis của daily returns thường vượt Gaussian 3, đôi khi rất lớn và không ổn định vì bị chi phối bởi vài extremes. Gaussian với constant volatility thường đánh giá thấp tail risk; Student-t/EVT là công cụ, không phải bảo đảm model đúng.
2. **Volatility clustering**: large absolute returns có xu hướng đi theo large absolute returns. Conditional volatility dự báo được hơn conditional mean, nhưng forecast vẫn có sai số lớn. Đây là chỗ GARCH và realized-vol models sống.
3. **Linear autocorrelation của liquid-asset returns thường nhỏ ở daily horizon**, nhưng có thể statistically/economically khác 0 theo asset và microstructure. Autocorrelation trong $|r|$ và $r^2$ thường dương và persistent hơn.
...
5. **Aggregational Gaussianity**: dưới weak dependence và finite variance, standardized aggregated returns có thể gần Gaussian hơn khi horizon tăng. Volatility clustering, jumps và regime mixtures làm convergence chậm; horizon dài không tự động an toàn hơn.
6. **Non-stationarity**: nhiều moments và quan hệ dự báo thay đổi theo regime. Rolling, expanding, decay-weighted và regime-aware estimators là các lựa chọn có trade-off; không phương pháp nào mặc định thắng.

Ngày 19/10/1987, S&P 500 giảm khoảng 20.5%. Nếu cứng nhắc lấy ex-ante daily volatility 1%, standardized move là khoảng 20.5 sigma và Gaussian constant-vol model gán xác suất thiên văn nhỏ. Con số sigma thay đổi nếu volatility estimate dùng window khác hoặc đã phản ứng với stress, nhưng kết luận bền vững là constant-vol Gaussian không mô tả được jumps, stochastic volatility và regime breaks. Sample kurtosis tóm tắt một phần tail thickness nhưng rất noisy; hai phân phối cùng kurtosis vẫn có thể cho tail probabilities khác nhau.

Gaussian 99% one-day VaR là $2.326\sigma$; với $\sigma=1\%$, threshold loss là 2.326%. Nếu model calibrated đúng, exceedance probability phải là 1%, khoảng 2.5 ngày trên 252 **theo kỳ vọng**. Không thể suy số exceedances 5–7 chỉ từ kurtosis 12: kurtosis không xác định riêng quantile 1%. Cách đúng là backtest exceedance rate/independence trên out-of-sample data và dùng distribution hoặc EVT model có assumptions được kiểm định.

Về **volatility clustering**, empirical ACF của raw daily returns thường nhỏ hơn nhiều ACF của absolute hoặc squared returns. Không có range 0.2–0.4 phổ quát: estimate phụ thuộc index/single-name, sampling, outlier treatment và lag. Ý đúng là dấu return có ít linear persistence hơn magnitude; vol forecast hữu ích nhưng không đồng nghĩa hướng "bất khả đoán" hay volatility được dự báo chính xác.

Về **leverage effect**, equity returns và future volatility innovations thường có quan hệ âm, nhưng magnitude phụ thuộc cách đo; không nên gắn một range correlation cố định. Cơ chế có thể gồm mechanical leverage, volatility feedback và risk aversion. Equity implied-vol skew liên quan hiện tượng này nhưng còn phản ánh risk-neutral crash premium, jump risk và option supply/demand; skew Q không chỉ là bản sao của correlation lịch sử P.

Với iid finite-fourth-moment returns, excess kurtosis của tổng standardized $n$ kỳ giảm theo $1/n$: daily excess kurtosis 9 sẽ thành 1.8 ở tổng 5 ngày và khoảng 0.43 ở tổng 21 ngày. Đây là benchmark toán, không phải forecast empirical. Volatility dependence, jumps và regime mixture làm convergence khác; không được tự gán weekly/monthly kurtosis 7 hay 4–5 nếu chưa đo. Tần suất thấp giảm turnover và một số microstructure noise, nhưng không miễn nhiễm tail risk.

Về **non-stationarity**, 2008 và tháng 3/2020 cho thấy volatility/correlation có thể đổi rất nhanh, nhưng exact multipliers phụ thuộc universe và estimator. Rolling windows thích nghi nhanh hơn nhưng tăng estimation noise; expanding windows ổn định hơn nhưng phản ứng chậm; regime models có model-selection risk. Không có lựa chọn mặc định luôn đúng. Sharpe cao qua nhiều regime là lý do để audit kỹ leakage, exposure và capacity, không tự nó là bằng chứng overfit.

## 2.4 Sampling: đồng hồ thời gian không phải đồng hồ duy nhất

Time bars (daily, 5-min) đơn giản và đồng bộ giữa assets nhưng activity trong mỗi bar không đều. Event-time bars là lựa chọn thay thế; chúng không mặc nhiên tốt hơn mà phụ thuộc signal, execution horizon và nhu cầu đồng bộ cross-asset:

- **Tick bars**: đóng bar mỗi $n$ giao dịch.
- **Volume bars**: mỗi $n$ đơn vị khối lượng — "đồng hồ" chạy theo hoạt động.
- **Dollar bars**: mỗi $n$ USD notional — invariant hơn volume bars trước thay đổi đơn vị giá/split, nhưng threshold vẫn cần cập nhật khi market-wide dollar volume đổi.
- **Imbalance bars**: đóng bar khi mất cân bằng mua/bán tích lũy vượt ngưỡng — đồng bộ với dòng thông tin.

Event bars có thể làm số observations co giãn theo activity và giảm intraday seasonality trong variance. Chúng **không bảo đảm iid hay Gaussian**, và volatility clustering không chỉ là hiện tượng của đồng hồ sai: information arrival, latent volatility và regime changes vẫn tồn tại sau time change. Chất lượng sampling phải được đo out-of-sample cho task cụ thể.

Ví dụ, volume bar một triệu cổ tương ứng 50 triệu USD khi price là 50 và 100 triệu USD khi price là 100. Dollar bar threshold 50 triệu USD cần một triệu cổ ở price 50 và 500 nghìn cổ ở price 100, nên ổn định hơn trước thay đổi price scale. Nhưng số bars/ngày vẫn tăng nếu total dollar turnover của thị trường tăng; threshold cố định không làm distribution stationary qua nhiều năm. Thực tế thường normalize threshold theo rolling ADV/dollar volume và khóa cách estimate theo PIT.

Activity time có thể homogenize phần volatility gắn với intraday trading intensity: khi thị trường bận, bars đóng nhanh hơn; lúc yên, đóng chậm hơn. Nhưng mỗi bar không chứa "cùng lượng thông tin" chỉ vì có cùng volume/notional, và endogenous order flow còn gây dependence. Vì vậy phải so sánh ACF, tails, sample count, synchronization và downstream P&L giữa time/tick/volume/dollar/imbalance bars thay vì tuyên bố một clock là thời gian thật duy nhất của thị trường.
...
**Bước 1 — Security master và mapping effective-dated.** Tạo internal IDs riêng cho legal entity, security và listing, rồi map ticker/CUSIP/ISIN/FIGI theo `valid_from/valid_to`. Corporate actions có thể đổi identifier hoặc quan hệ giữa entity và security, nên security master không phải bài toán giải một lần rồi xong; nó là versioned reference dataset cần reconciliation liên tục.
...
**Bước 4 — Join theo source availability.** Mọi version cần ít nhất `period_end` (hoặc event time tương ứng), `source_available_at` và `system_ingested_at`. Hypothetical PIT backtest tại $t$ chỉ dùng version có `source_available_at\le t`; replay production lịch sử mới yêu cầu thêm `system_ingested_at\le t`. Với vendor correction/restatement, không overwrite bản cũ. Alt-data phải mô hình hóa publication lag, vendor delivery lag và revision policy riêng.

**Bước 5 — Temporal leakage tests.** Ground truth tốt nhất là snapshot hoặc production signal thực sự được lưu tại $t_0$; replay hôm nay bằng versioned store phải tái tạo nó trong tolerance đã định. Một test khác là thêm các rows có `source_available_at>t_0` rồi assert mọi feature/signal tại $t\le t_0$ bất biến. Nếu historical output thay đổi, có leakage, nondeterminism hoặc upstream revision chưa được version hóa. Chỉ chạy lại current vendor database "as if" ở $t_0$ không chứng minh PIT sạch, vì database hiện tại có thể đã overwrite lịch sử.
...
Bộ công cụ tay nghề của quant researcher. Trọng tâm không phải "biết công thức" mà là **biết estimand, assumptions và failure mode**. Không phải mọi t-stat đều giả IID/homoskedasticity — robust/HAC/clustered inference tồn tại chính để thay covariance assumptions — nhưng software default thường ngầm dùng điều kiện quá sạch so với dữ liệu tài chính. Chương này đi qua ước lượng, kiểm định, hồi quy, time series, cointegration và state space với cùng câu hỏi: *con số này đang ước lượng gì, đúng dưới điều kiện nào, và sai lệch đó có làm đổi quyết định giao dịch không?*
...
**Mean return rất khó ước lượng từ lịch sử.** Dưới benchmark IID/stationary, ước lượng annual mean return $\mu$ từ $T$ năm dữ liệu có sai số chuẩn xấp xỉ $\sigma/\sqrt{T}$. Với $\sigma = 16\%$/năm: để SE tụt xuống $1.6\%$ cần $\sqrt{T}=10$, tức $T=100$ **năm**. Serial dependence, regime change và parameter drift thường làm tình hình tệ hơn benchmark này. Volatility và correlation ngắn hạn thường dễ ước lượng hơn mean, nhưng không vì thế mà trở thành các hằng số biết chắc.

Vì sao mean khó mà vol dễ? Giả sử interval return có độ dài $\Delta=T/n$ năm, mean xấp xỉ $\mu\Delta$ và variance xấp xỉ $\sigma^2\Delta$. Annualized sample mean là $\hat\mu=(n\Delta)^{-1}\sum_i r_i$, nên

$$SE(\hat\mu)\approx\frac{\sigma}{\sqrt{n\Delta}}=\frac{\sigma}{\sqrt T}.$$

Tăng sampling frequency làm $n$ tăng nhưng variance của mỗi interval giảm cùng tỷ lệ, nên không tạo thêm calendar history cho drift. Chỉ với **log returns**, tổng return qua cả mẫu mới rút gọn chính xác về $\log(P_T/P_0)$; arithmetic sample mean không chỉ phụ thuộc hai endpoint. Ngược lại, dưới giả định Gaussian IID với volatility không đổi, relative SE của sample volatility xấp xỉ $1/\sqrt{2n}$. Với $n\approx7560$, benchmark lý tưởng là khoảng $0.8\%$. Đây **không** có nghĩa ta biết future volatility chính xác dưới 1%: volatility clustering và regime drift làm effective sample size thấp hơn và chính tham số cần dự báo cũng thay đổi theo thời gian.

Con số stylized để cảm nhận: giả sử annual excess return có mean $8\%$ và volatility $16\%$. Với 30 năm dữ liệu, $SE(\hat\mu)=16\%/\sqrt{30}=2.92\%$. Khoảng tin cậy Gaussian 95% là $8\%\pm1.96\times2.92\%$, tức khoảng **2.3% đến 13.7%**. Đây là minh họa toán học, không phải tuyên bố rằng equity premium thật của S&P 500 đúng bằng 8%; estimate thực còn phụ thuộc sample, risk-free proxy, inflation convention và regime.

**Sai số của chính Sharpe ratio.** Phải giữ sampling frequency trong công thức. Giả sử có $q$ IID Gaussian observations mỗi năm, tổng cộng $n=qT$ observations, và $SR$ là Sharpe **annualized**. Delta method cho

$$SE(\widehat{SR}_{ann})\approx\sqrt{\frac{1+SR_{ann}^2/(2q)}{T}}.$$

Nếu chỉ có annual returns ($q=1$), công thức rút về $\sqrt{(1+SR^2/2)/T}$. Nếu tính từ daily IID returns ($q=252$), chiến lược annualized Sharpe 1 trên 5 năm có $SE\approx\sqrt{(1+1/504)/5}=0.448$, cho naive 95% interval khoảng $[0.12,1.88]$. Đây vẫn là dải rất rộng. Quan trọng hơn, Lo (2002) chỉ ra rằng serial correlation làm cả annualization lẫn SE thay đổi; với smoothing, overlapping positions hoặc illiquid marks, phải dùng HAC/block bootstrap thay vì cắm máy móc $q=252$.

Để test null **mean excess return bằng 0**, dưới IID benchmark ta dùng $t\approx\widehat{SR}_{ann}\sqrt T$. Vì vậy true Sharpe 1 cần khoảng 4 năm để expected $t$ chạm 2; Sharpe 0.5 cần khoảng 16 năm. Đây là power calculation lý tưởng cho một giả thuyết định trước, không phải bảo đảm phát hiện: non-normality, dependence, parameter drift và multiple testing đều làm bằng chứng yếu đi.
...
Đây là chỗ trực giác của người mới sụp đổ. Nếu thử $N$ chiến lược độc lập, mỗi t-stat theo $N(0,1)$ dưới null, expected maximum là

$$E[M_N]=\int z\,N\phi(z)\Phi(z)^{N-1}\,dz.$$

Xấp xỉ bậc đầu $\sqrt{2\ln N}$ hội tụ rất chậm và **không** nên dùng như expected maximum ở $N$ vừa phải. Blom quantile approximation $E[M_N]\approx\Phi^{-1}((N-0.375)/(N+0.25))$ cho khoảng 1.87 khi $N=20$, 2.50 khi $N=100$, và 3.23 khi $N=1000$. Một cách nhìn khác: với 100 tests độc lập, xác suất có ít nhất một kết quả vượt ngưỡng two-sided 5% là $1-0.95^{100}\approx99.4\%$. Correlation giữa variants làm effective $N$ nhỏ hơn raw count, nhưng không xóa selection bias.

Harvey–Liu–Zhu (2016) khảo sát "factor zoo" và đề nghị hurdle t-ratio lớn hơn 3 cho khám phá mới. Con số này đến từ một multiple-testing framework có correlation giữa tests và publication bias, không thể diễn giải ngược đơn giản thành "đúng 90 thí nghiệm ẩn". Chương 9 đưa công cụ định lượng hơn — **Deflated Sharpe Ratio** — nhưng DSR còn cần số trials hiệu dụng, variance giữa trials, sample length, skewness và kurtosis; chỉ biết $N$, $T$ và Sharpe quan sát chưa đủ để tuyên bố một xác suất duy nhất.

Nguyên tắc vận hành rút ra từ toán này: **đếm mọi thí nghiệm, kể cả thất bại**. Với 50 independent null variants, expected max t chỉ khoảng 2.24 theo Blom approximation — thấp hơn $\sqrt{2\ln50}=2.80$ nhưng vẫn đủ tạo một backtest trông đáng kể. Thực tế variants tương quan và researcher còn thay universe, cleaning, cost model, metric; vì vậy cần experiment registry, holdout thật và multiple-testing adjustment thay vì chỉ đếm số file backtest cuối cùng.

## 3.3 Stationarity, spurious regression, và bộ đôi ADF/KPSS

**Stationarity.** Strict stationarity đòi toàn bộ joint distribution bất biến theo phép dịch thời gian; weak stationarity chỉ đòi mean, variance và autocovariance phụ thuộc lag chứ không phụ thuộc calendar time. Giá thường được mô hình hóa là không dừng; returns thường gần weak-stationary hơn nhưng vẫn có conditional heteroskedasticity và regime change. Với arithmetic random walk $P_t=P_{t-1}+\epsilon_t$, $\operatorname{Var}(P_t)=t\sigma^2$ trong khi increment $\Delta P_t=\epsilon_t$ có variance cố định — ví dụ sạch cho khác biệt I(1) và I(0).

Vì sao level không dừng nguy hiểm? Hồi quy hai I(1) series không cointegrated có thể tạo **spurious regression**: $R^2$ và conventional t-stat lớn dù innovations độc lập. Các con số cụ thể thay đổi mạnh giữa simulation paths, nên không có một "R² trung bình chuẩn" để nhớ. Cơ chế mới quan trọng: residual vẫn I(1), trong khi công thức OLS cổ điển giả residual có second moments ổn định; vì vậy standard t/F distributions không còn hợp lệ và apparent significance có thể tăng cùng sample length.

Ví dụ minh họa: với hai random walk độc lập dài 500 phiên, hoàn toàn có thể gặp một realization cho $\hat\beta=0.6$, conventional t-stat $8.1$ và $R^2=0.42$; seed khác sẽ cho bộ số khác. Hồi quy increments của cùng hai paths thường đưa hệ số và $R^2$ về gần 0. Bài học không phải "differencing luôn là đáp án" — nếu series cointegrated, differencing riêng lẻ làm mất quan hệ dài hạn — mà là inference trên levels phải bắt đầu bằng integration/cointegration diagnosis.

Quy tắc thực dụng: coi level regression là không hợp lệ **cho tới khi** integration/cointegration framework chứng minh ngược lại; returns hoặc stationary spreads thường an toàn hơn cho conventional inference. Hai kiểm định bổ sung nhau:

- **ADF (Augmented Dickey–Fuller)** — null là "có unit root". Nó hồi quy $\Delta y_t=\phi y_{t-1}+\sum_i\delta_i\Delta y_{t-i}+\epsilon_t$ cùng deterministic terms phù hợp rồi test $\phi=0$. Price levels thường không bác null còn returns thường bác mạnh, nhưng statistic cụ thể phụ thuộc sample và lag selection. Phân phối ADF không phải Student-t; giá trị tới hạn 5% cỡ $-2.86$ chỉ áp cho một specification lớn mẫu có intercept, không trend. Đổi deterministic terms sẽ đổi critical values.
- **KPSS** — null là level-stationary hoặc trend-stationary tùy deterministic specification. Dùng ADF và KPSS cùng nhau giúp nhìn từ hai null đối nghịch: ADF bác + KPSS không bác ủng hộ stationarity; ADF không bác + KPSS bác ủng hộ unit-root/nonstationarity; hai test cùng không bác hoặc cùng bác báo power/specification conflict, structural break hoặc fractional behavior cần đào sâu. Không được trộn ADF-with-trend với KPSS-level rồi đọc như một cặp đối xứng.
...
OLS: $y=X\beta+\epsilon$, $\hat\beta=(X^\top X)^{-1}X^\top y$. Trong nghề, hồi quy dùng cho beta/factor exposure, kiểm tra alpha, neutralization và baseline forecasting. Linear model với feature tốt thường là baseline rất khó đánh bại out-of-sample, nhưng không có định lý rằng nó gần như luôn thắng model phức tạp; kết luận phải đến từ validation đúng chronology và costs.

Chỗ gãy trên dữ liệu tài chính và cách vá — bảng này đáng giá hơn cả chục trang lý thuyết:

| Vấn đề | Hậu quả | Vá chuẩn industry |
|---|---|---|
| Heteroskedasticity (vol clustering) | Conventional SE sai | HC/White SE; HC3 hữu ích ở mẫu nhỏ |
| Autocorrelation của residuals (overlapping returns) | Conventional SE thường quá nhỏ | HAC/Newey–West hoặc Hansen–Hodrick; non-overlapping/block bootstrap để kiểm tra |
| Outliers (đuôi dày) | $\hat\beta$ bị vài điểm kéo lệch | Audit data, transform/winsorize theo rule PIT, robust regression |
...
| Cross-sectional correlation (mọi cổ phiếu cùng nhảy theo market) | Effective N có thể nhỏ hơn raw N rất nhiều | Multiway/clustered SE; Fama–MacBeth khi estimand phù hợp (chương 5) |

Bảng này chỉ nói "vá thế nào"; điều quan trọng là cảm nhận *giá của việc không vá bằng con số*. Lấy ba chỗ gãy nguy hiểm nhất.

**Heteroskedasticity và White SE.** Nếu $E[\epsilon\mid X]=0$, OLS vẫn unbiased/consistent dù conditional variance thay đổi; heteroskedasticity không cứu được endogeneity, omitted variables hay look-ahead. Conventional covariance $\hat\sigma^2(X^\top X)^{-1}$ sai khi $\operatorname{Var}(\epsilon_t\mid X)$ không hằng. White estimator dùng sandwich

$$\widehat{\operatorname{Var}}(\hat\beta)=(X^\top X)^{-1}\left(\sum_t\hat\epsilon_t^2x_tx_t^\top\right)(X^\top X)^{-1}.$$

Residuals không "thống trị $X^\top X$" — chúng đi vào phần giữa của sandwich; regressors mới tạo $X^\top X$. Mức thay đổi của t-stat không có range 20–50% phổ quát: nó phụ thuộc việc variance liên hệ với leverage của $x_t$. HC estimators sửa large-sample inference dưới heteroskedasticity, không sửa coefficient bias do specification sai.

**HAC và cái bẫy overlapping returns.** Với 21-day forward returns sampled daily, hai targets kề nhau chia sẻ 20 one-day returns. Trong benchmark one-day innovations IID, autocorrelation do overlap tại lag $k<21$ bằng $(21-k)/21$. Long-run variance của sample mean lớn hơn naive variance đúng hệ số 21, nên naive t-stat có thể lớn hơn khoảng $\sqrt{21}=4.58$ trong toy case intercept-only. Trong regression thật, predictors, residual dynamics và conditional heteroskedasticity làm correction khác con số này.

Không có quy tắc chuẩn "lag = 1.5 lần horizon". Với overlap cơ học $h$, covariance estimator ít nhất phải nhận diện dependence tới $h-1$ lags; Hansen–Hodrick khai thác trực tiếp cấu trúc overlap, còn Newey–West/HAC cần kernel và bandwidth được biện minh hoặc chọn tự động. So sánh với non-overlapping estimates và block bootstrap là sanity check tốt. HAC sửa covariance estimate, không biến overlapping sample thành $T/h$ observations theo nghĩa literal và không bảo đảm kéo một t-stat cụ thể về đúng một con số định trước.

**Cross-sectional correlation và N hiệu dụng.** Trong toy model equal-weight/equal-correlation, $N_{eff}=N/[1+(N-1)\rho]$, nên $N=3000,\rho=0.3$ cho giới hạn gần $1/\rho\approx3.3$. Đây không phải estimate phổ quát: factor controls, sector structure và unequal weights thay đổi mạnh effective dimension. Pooled OLS vẫn có thể dùng nếu covariance estimator phản ánh dependence đúng, nhưng naive iid SE thì không.

Fama–MacBeth chạy cross-sectional regression mỗi kỳ, rồi lấy mean của chuỗi coefficient $\hat\beta_t$. Nó cho inference robust với arbitrary contemporaneous cross-sectional correlation dưới asymptotics phù hợp, nhưng **không tự động miễn nhiễm mọi dependence**: nếu $\hat\beta_t$ autocorrelated, SE của mean vẫn cần HAC; estimated betas/measurement error còn có thể cần Shanken-type hoặc errors-in-variables adjustment.

**Shrinkage — một tư tưởng trung tâm của P-world.** Ridge/Lasso/Bayes/Ledoit–Wolf đổi một phần bias lấy variance thấp hơn; chúng thường giúp out-of-sample khi signal yếu, nhưng target và cường độ shrink vẫn phải validate. James–Stein cho một kết quả chính xác hơn: với vector Gaussian có covariance $\sigma^2I$ và squared-error risk, sample mean vector là inadmissible từ dimension 3 trở lên khi shrink về một **target cố định**. Nếu target là sample grand mean, ta đã ước lượng thêm một nuisance parameter nên hệ số đổi từ $k-2$ thành $k-3$ và nontrivial dominance bắt đầu từ $k\ge4$:

$$\hat\mu_i^{JS+}=\bar\mu+\left(1-\frac{(k-3)\sigma^2}{\sum_j(\hat\mu_j-\bar\mu)^2}\right)_+(\hat\mu_i-\bar\mu),$$

kéo mỗi estimate về grand mean; $(x)_+=\max(x,0)$ là positive-part để tránh "shrink xuyên qua target". Dominance là về **expected total squared-error risk dưới assumptions trên**, không phải mọi coordinate hay mọi realized sample đều tốt hơn.

Trong toy sample $k=10$, true means đều 5%, estimate là $(1,2,3,4,5,5,6,7,8,9)\%$, và SE mỗi estimate là 2%. Ta có dispersion $60\ \%^2$ và factor $1-7\times4/60=0.533$. Estimate 9% co về $5+0.533\times4=7.13\%$; 1% lên 2.87%. Vì target tình cờ đúng bằng truth trong toy này, realized SSE giảm từ 60 xuống $0.533^2\times60\approx17.1$, tức khoảng 72%. Đây là illustration thuận lợi, không phải mức cải thiện được bảo đảm cho mọi dataset. Trong covariance estimation, shrinkage target và intensity cũng phải được chọn/ước lượng thay vì mặc định structure luôn đúng.

## 3.5 Time series models: ARMA, GARCH, và realized volatility

**ARMA.** AR($p$) dùng lags của series; MA($q$) dùng lags của innovations. Với broad liquid-asset daily returns, linear autocorrelation thường nhỏ và không ổn định, nên ARMA mean forecast là baseline khó tạo economic edge sau costs. Nhưng microstructure, stale prices, lead–lag, intraday seasonality và một số spreads/order-flow series vẫn có predictability. Dưới white-noise null, approximate SE của sample autocorrelation là $1/\sqrt T$; với $T=2500$, SE khoảng 0.02, nên sample $\hat\rho_1=0.03$ chỉ cỡ 1.5 SE. Absolute/squared returns thường persistent hơn raw returns, nhưng magnitude phụ thuộc asset, horizon và sample — không có range ACF phổ quát.
...
"Conditional variance hôm nay = intercept + shock bình phương hôm qua + quán tính variance hôm qua". $\epsilon_{t-1}$ là mean-model residual; chỉ khi conditional mean được đặt bằng 0 mới thay trực tiếp bằng raw return. $\omega$ là intercept, còn unconditional variance (nếu $\alpha+\beta<1$) mới bằng $\omega/(1-\alpha-\beta)$. GJR-GARCH và EGARCH cho phép shock âm/dương tác động khác nhau. Parameter estimates và distribution của standardized innovations phải được kiểm tra thay vì coi $(0.08,0.90)$ là universal constants.
...
- **Sau một cú sốc.** Giả sử mean bằng 0, hôm qua return −3% và conditional vol trước đó 1%: $\sigma_t^2=2\times10^{-6}+0.08(0.03)^2+0.90(0.01)^2=1.64\times10^{-4}$, nên forecast vol là 1.28%/ngày, khoảng 20.3% annualized dưới square-root scaling. Symmetric GARCH dùng cùng kết quả cho +3%; muốn downside shock mạnh hơn phải dùng asymmetric model.
- **Cú sốc phai nhanh thế nào.** Độ lệch của **variance forecast** khỏi unconditional variance phai theo $(\alpha+\beta)^n=0.98^n$ trong GARCH(1,1), cho half-life $\ln(0.5)/\ln(0.98)\approx34$ trading days. Tần suất vol targeting không suy ra trực tiếp từ half-life: weekly, daily hay monthly còn phụ thuộc turnover, gap risk, leverage rule và execution costs.

**Realized variance và HAR.** Đại lượng

$$RV_t=\sum_{i=1}^{m}r_{t,i}^2$$

là **realized variance**; realized volatility là $\sqrt{RV_t}$. Với 5-minute sampling trong phiên Mỹ, $m=78$. Trong toy model intraday Gaussian IID với variance không đổi và không có market-microstructure noise, relative SE của sum-of-squares giảm theo $1/\sqrt m$, nên RV chứa nhiều thông tin hơn một daily squared return. Ngoài đời, intraday returns không độc lập, overnight variance bị bỏ sót, jumps tồn tại, và sampling quá dày gây bid–ask/microstructure bias; 5-minute là convention thực dụng chứ không phải optimum phổ quát.

Họ **HAR** của Corsi (2009) thường được fit trên realized volatility, realized variance hoặc log của chúng. Dưới convention ở đây ta dùng realized variance và viết ba horizon — ngày, tuần, tháng:

$$RV_{t+1} = c + \beta_d\, RV_t + \beta_w\, \overline{RV}_t^{(5)} + \beta_m\, \overline{RV}_t^{(22)} + \epsilon_t$$

trong đó $\overline{RV}^{(5)}$ và $\overline{RV}^{(22)}$ là averages 5 và 22 ngày. Diễn giải bằng heterogeneous market participants là motivation kinh tế, không phải identification result. HAR thường là benchmark dự báo rất mạnh khi có high-frequency data, nhưng không mặc nhiên thắng GARCH ở mọi asset/loss function; nên so sánh out-of-sample bằng cùng target, horizon và QLIKE/MSE phù hợp.

## 3.6 Cointegration, Ornstein-Uhlenbeck, và half-life — nền của pairs trading

**Cointegration — nền của pairs trading.** Nếu $X_t$ và $Y_t$ đều I(1) nhưng tồn tại $Y_t-c-\gamma X_t$ là I(0), chúng cointegrated. Đây là điều kiện thống kê phải test, không thể suy từ câu chuyện "cùng ngành": Coca-Cola/Pepsi chỉ là candidate, trong khi dual-class shares hoặc ETF/basket có linkage kinh tế chặt hơn nhưng vẫn chịu frictions và breaks.

**Engle-Granger 2 bước** — recipe cụ thể làm lại được:
1. Xác nhận mỗi series có integration order phù hợp; hồi quy level $Y_t=c+\gamma X_t+u_t$ với intercept/trend được chọn theo economics.
2. Test unit root trên estimated residual $\hat u_t$. Phải dùng **Engle–Granger/MacKinnon critical values**, không dùng bảng ADF thông thường, vì residual được tạo từ first-stage estimated coefficients. Bác null no-cointegration nghĩa là có bằng chứng cointegration trong sample; fail to reject không chứng minh hai innovations độc lập, chỉ nói test chưa tìm thấy stationary residual.
...
$\kappa$ là continuous-time mean-reversion speed và half-life của conditional mean là $\ln2/\kappa$. Exact discrete transition là

$$s_t=\mu+\phi(s_{t-1}-\mu)+\eta_t,\qquad \phi=e^{-\kappa\Delta t}.$$

Nếu hồi quy $\Delta s_t=a+b s_{t-1}+\epsilon_t$, thì $\phi=1+b$ và

$$\kappa=-\frac{\ln(1+b)}{\Delta t},\qquad h_{1/2}=\frac{\ln(0.5)}{\ln\phi}\ \text{periods}.$$

Quy tắc $\kappa\approx-b/\Delta t$ chỉ là Euler approximation khi $|b|$ nhỏ. Cũng phải phân biệt OU diffusion coefficient $\sigma$ với stationary standard deviation của spread: $\sigma_s=\sigma/\sqrt{2\kappa}$ trong continuous-time stationary OU.

**Half-life OU bằng số.** Nếu exact fit cho $\phi=e^{-0.05}=0.9512$ mỗi ngày, thì regression trên $\Delta s_t$ có $b=\phi-1=-0.0488$ và half-life $\ln2/0.05=13.9$ ngày. Giả sử **stationary standard deviation** của spread là $\sigma_s=2.4\%$. Một policy minh họa, chưa tính costs, borrow và break risk:

- **Vào lệnh** ở $|z|>2$ tương ứng lệch hơn $4.8\%$ trong toy scale này. Threshold 2 không có optimality phổ quát; phải fit trên training data và validate net-of-cost.
- **Conditional mean** đi từ z = 2 về z = 1 sau một half-life và về z = 0.5 sau hai half-lives. Đây không phải expected first-passage/holding time; thời gian thật để chạm exit threshold là biến ngẫu nhiên và phải tính từ hitting-time distribution hoặc simulation.
- **Thoát** ở z = 0 (hoặc ±0.5 để chốt sớm, giảm rủi ro đảo chiều).
- **Stop** ở một threshold như 3.5–4 chỉ là risk rule minh họa, không phải output của OU. OU Gaussian thuần coi extreme deviation là cơ hội mean-reversion mạnh hơn; stop tồn tại vì model-break, funding và gap risk nằm ngoài OU.

**Half-life là một filter, không phải verdict.** Half-life dài có thể làm capital turnover thấp; half-life rất ngắn có thể đẩy turnover/cost lên cao. "Vùng ngọt 5–30 ngày" phụ thuộc execution horizon, liquidity và signal design. Estimate gần unit root cũng rất bất ổn, nên phải kèm confidence interval, out-of-sample stability và expected P&L sau costs.

**Cảnh báo nghề — cointegration gãy được.** Tháng 8/2007 cho thấy crowded stat-arb books có thể cùng deleverage và làm spreads giãn mạnh. Rolling Engle–Granger/ADF hữu ích nhưng có low power và repeated-testing noise; "không còn bác" ở một window không tự động chứng minh break. Production monitoring nên kết hợp parameter drift, structural-break tests, liquidity/borrow, residual co-movement và crowding, với policy de-risk được định trước.
...
$F$ là ma trận chuyển trạng thái; $H_t$ là ma trận quan sát; $Q,R$ là covariance của state và measurement noise. Standard Kalman recursions còn giả $w_t,v_t$ white, mutually uncorrelated và uncorrelated với initial state; nếu có cross-covariance phải dùng công thức tổng quát hơn. Khung này bao trùm dynamic regression, local level/trend, seasonal decomposition và ARMA state-space representation.

**Kalman filter** là linear minimum-MSE estimator từ first/second moments; trong linear-Gaussian model nó cũng là full posterior mean và covariance. Viết trường hợp scalar dynamic regression $Y_t=c_t+\gamma_tX_t+\epsilon_t$ sau khi đã center data hoặc đưa intercept $c_t$ vào state. Bỏ intercept mà dùng raw price levels có thể biến scale/trend thành hedge-ratio movement giả; với pairs vẫn phải kiểm tra spread/cointegration economics. Các công thức dưới đây là centered case $c_t=0$.
...
Innovation $Y_t-H_t\hat\theta_{t|t-1}$ điều khiển update. $Q$ lớn cho phép state trôi nhanh; $R$ lớn làm filter ít tin observation. Nhưng $Q/R$ chỉ có ý nghĩa sau khi cố định units/scaling của state và measurement matrix $H_t$; rescale $X_t$ sẽ rescale $\gamma,P,Q$ dù fitted $Y$ không đổi. $Q,R$ thường được estimate bằng likelihood/EM hoặc tuned out-of-sample, không chọn chỉ bằng trực giác.

**Kalman bằng số — một bước cập nhật cụ thể.** Trong toy relationship qua gốc với units đã cố định, giả sử $\hat\gamma_{t-1}=1.50$, $P_{t-1}=0.01$, $Q=0.0001$, $R=0.04$, $X_t=30$ và $Y_t=47.4$.
...
Đọc kết quả: với units đã chọn, $K_tX_t\approx0.996$ nên observation gần như xác định state của bước này; posterior variance co xuống khoảng $4.4\times10^{-5}$. Điều đó không có nghĩa dữ liệu kinh tế tự nhiên "chắc chắn" đến 0.0066: uncertainty này **conditional on model, $Q,R$, units và absence of parameter/model break**. Kalman quên mượt hơn rolling window và cung cấp model-based uncertainty, nhưng position sizing vẫn cần residual risk, parameter uncertainty, liquidity và stress overlays.
...
**VECM (Vector Error Correction Model)** là dạng đa biến của "spread kéo về cân bằng". Với vector $\mathbf{y}_t$ và underlying VAR order $k$, VECM viết:

$$\Delta \mathbf{y}_t = \Pi\,\mathbf{y}_{t-1} + \sum_{i=1}^{k-1}\Gamma_i\,\Delta\mathbf{y}_{t-i} + \boldsymbol\epsilon_t$$

Trái tim nằm ở ma trận $\Pi$. Nếu $\operatorname{rank}(\Pi)=0$, không có error-correction/cointegration term; các differences vẫn có thể correlated và predictable qua $\Gamma_i$, nên không được gọi các random walks là độc lập. Nếu $\operatorname{rank}(\Pi)=n$, level vector là stationary dưới specification phù hợp. Trường hợp $0<r<n$ cho $\Pi=\alpha\beta^\top$: columns của $\beta$ span cointegration space, còn $\alpha$ chứa loading/tốc độ các biến phản ứng với disequilibria. $\alpha$ không phải một scalar $-\kappa$; nhiều spreads và cross-adjustments làm dynamics phong phú hơn OU một chiều.

**Johansen test** ước lượng VECM bằng Gaussian reduced-rank maximum likelihood và giải một generalized eigenvalue problem từ residual covariance matrices. Các eigenvalues $\hat\lambda_1\ge\cdots\ge\hat\lambda_n$ là squared canonical correlations liên quan tới error-correction directions, không đơn giản là raw eigenvalues của một $\Pi$ OLS bất kỳ. Trace statistic là

$$J_{trace}(r)=-T\sum_{i=r+1}^{n}\ln(1-\hat\lambda_i),$$

với null $\operatorname{rank}(\Pi)\le r$; maximum-eigenvalue test so rank $r$ với $r+1$. Lag order, intercept/trend placement, seasonality và breaks ảnh hưởng mạnh cả statistic lẫn critical values.

**Johansen bằng số — ví dụ giả lập giống yield curve.** Giả sử ba I(1) series cho generalized eigenvalues $(0.08,0.03,0.002)$ với $T=1000$. Trace statistics lần lượt là $115.9$, $32.5$ và $2.0$. Với một deterministic specification có 5% critical values minh họa $29.8$, $15.5$, $3.8$, ta bác rank 0 và rank $\le1$ nhưng không bác rank $\le2$, nên chọn $r=2$. Số học đúng, nhưng kết luận không được nâng thành định luật rằng 2Y/5Y/10Y yields luôn I(1) với rank 2: unit-root và rank estimates của yields nhạy với sample, monetary regime, lags và deterministic terms. Estimated $\beta$ vectors cũng không tự động trùng đúng 2s10s slope và 2s5s10s butterfly; normalization và loadings phải được đọc từ output.

Johansen và PCA trả lời hai câu khác nhau. Johansen tìm stationary linear combinations dưới I(1)/VECM assumptions; PCA tìm orthogonal directions giải thích sample variance, không áp stationarity constraint. Trên yield curves cả hai thường gợi level/slope/curvature, nhưng PCA eigenvectors không phải cointegration vectors theo định nghĩa. Production use cần stability checks, break monitoring và economic normalization; rolling test liên tục cũng tạo repeated-testing noise nên phải đi cùng policy rõ ràng, không phải cứ p-value đổi là bật/tắt trade.

---

Điểm chung xuyên suốt chương: một con số econometric chỉ có nghĩa khi đi cùng **estimand, sampling scheme, covariance model và stability assumption**. Mean return hội tụ chậm theo calendar time; overlapping 21-day targets có thể làm naive t-stat phồng tới $\sqrt{21}$ trong toy IID case; 100 null tests gần như chắc chắn sinh ít nhất một conventional 5%-significant result; level regression có thể spurious; cointegration rank và half-life có thể đổi theo regime. Không có một miếng vá duy nhất: HC/HAC/clustered SE sửa inference chứ không sửa endogeneity; shrinkage giảm variance nhưng cần target; Kalman thích nghi tham số nhưng vẫn conditional on model; rolling tests tự tạo repeated-testing noise. Tinh thần mang sang chương 4 là: **định nghĩa đúng câu hỏi trước, rồi mới chọn estimator và uncertainty phù hợp.**
