...
import { hwInstantaneousForward, hwAlpha } from './hull-white-affine'
...
export function hullWhiteBond(spec: HullWhiteSpec, t: number, maturity: number, shortRate: number): number {
  ...
  const b = affineBFactor(a, maturity - t)
  const ratio = spec.curve.discountFactor(maturity) / spec.curve.discountFactor(t)
  const logA = Math.log(ratio) + b * hwInstantaneousForward(spec.curve, t) - (spec.vol * spec.vol) / (4 * a) * (1 - Math.exp(-2 * a * t)) * b * b
...
export function zeroBondPut(spec: HullWhiteSpec, expiry: number, bondMaturity: number, strike: number): number {
  const a = spec.meanReversion
  const sigmaP = spec.vol * affineBFactor(a, bondMaturity - expiry) * Math.sqrt((1 - Math.exp(-2 * a * expiry)) / (2 * a))
...
export function hullWhitePayerSwaptionMc(spec: HullWhiteSpec, swaption: HullWhiteSwaption, steps: number, paths: number, seed: number): number {
  ...
  for (let p = 0; p < paths; p += 1) {
    ...
    for (let step = 0; step < steps; step += 1) {
      const time = step * dt
      integral += (x + hwAlpha(spec.curve, a, spec.vol, time)) * dt
      x = x * decay + stepStd * inverseNormalCdf(generator.nextDouble())
    }
    const shortRate = x + hwAlpha(spec.curve, a, spec.vol, swaption.expiry)
